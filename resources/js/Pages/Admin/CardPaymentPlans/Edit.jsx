...
export default function Edit({ plan }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        put(route('admin.card-payment-plans.update', plan.id), {
            onSuccess: () => toast.success('Plan de cuotas actualizado exitosamente'),
    ...
    return (
            ...
            header={
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="min-w-0">
                        ...
                        <h1 className="mt-1 truncate text-xl font-semibold text-slate-900 sm:text-2xl">{plan.name}</h1>
                    </div>
                    <Link
        ...
        >
            <Head title={`Editar ${plan.name} - Admin`} />

            <div className="mx-auto max-w-2xl space-y-4">
                {plan.en_uso && (
                    <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                        Este plan ya se usó en órdenes. Podés editarlo, pero no se puede eliminar: si querés dejar de
                        ofrecerlo, desactivalo.
                    </div>
                )}
